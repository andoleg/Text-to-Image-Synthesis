...
class generator_loss(torch.nn.Module):
    ...
    def forward(self, fake):
        ...
        self.labels = Variable(torch.FloatTensor(batch_size).to(self.device).fill_(1))
...
class discriminator_loss(torch.nn.Module):
    ...
    def forward(self, real, wrong, fake):
        ...
        self.real_labels = Variable(torch.FloatTensor(batch_size).to(self.device).fill_(1))
        self.fake_labels = Variable(torch.FloatTensor(batch_size).to(self.device).fill_(0))
